...
throw new \InvalidArgumentException("Crop rectangle does not fit within image data.");
  ...
  public function getRow($y, $row=null) {
    ...
    $offset = ($y + $this->top) * $this->dataWidth + $this->left;
      $row = array_copy($this->yuvData, $offset, $row, 0, $width);
    return $row;
  }
  ...
  public function getMatrix() {
    ...
    if ($width == $this->dataWidth) {
        $matrix = array_copy($this->yuvData, $inputOffset, $matrix, 0, $area);
        return $matrix;
    }
    ...
    $outputOffset = $y * $width;
      $matrix = array_copy($this->yuvData, $inputOffset, $matrix, $outputOffset, $width);
